...
% --------------------
% Sorting takes place using a factor that depends only on time; for example business cycle is the same for all stocks so there is perfect cross-correlation and we cannot use the cross-correlation because there is no variation. 
% --------------------
\begin{theorem}
...
\noindent
The theorem states that under the assumption of strong cross-sectional correlation of the sorting variable there is an additional term in the asymptotic covariance matrix. %This is because the difference between the order statistic and the quantile is of order $1/\sqrt{T}$, rather than $1/(\sqrt{ {T}}N^{1-\rho })$ with $1/2\leq \rho <1$ as in the weak cross-sectional correlation case. Intuitively, since the sorting variables are highly correlated, we can use only the time dimension to estimate the quantile of the order statistics and the cross-sectional dimension no longer contributes to the convergence.
Intuitively, under weak cross-sectional dependence, the cross-section helps estimate the cutoff: the order-statistic error shrinks with \(N\), so its normalized time-summed contribution is \(o_p(1)\). Under strong cross-sectional dependence, a common component makes the cutoff errors move together across stocks, so the cross-section no longer provides an additional rate of convergence. The term \((T-\mathcal T)^{-1/2}\sum_t (S_{\tau(t)}^{p(j)}-S^{q(j)})\) is no longer negligible and must enter the asymptotic variance.
...
\begin{figure}[H]
	\centering
 	...
 	\end{subfigure}
 	\caption[Contamination error and information loss error]{Contamination error and information loss error for portfolio 1.}
...
\begin{figure}[H]
	\centering
 	...
 	\end{subfigure}
 	\caption[Trimming procedure]{Trimming procedure.}
	\label{img:trimming}
\end{figure}

\subsection{Asymptotic Results}

We now define the feasible and trimmed counterpart of $Z_{T,N}$. Moreover, in order to properly control for sorting errors, we need three additional assumptions.
...
\begin{enumerate}[(i)]
\item Under $H_{0}$, if $\frac{T_\delta}{T}\rightarrow 0$, and $\frac{T-R}{R}\rightarrow \pi$ and $\frac{T_\delta-R_\delta}{R_\delta}\rightarrow \pi$ for $0<\pi<\infty$ as $T,N,R,T_\delta,K,R_\delta\rightarrow \infty$:
\begin{align*}
...
\end{align*}
We estimate the asymptotic variance of the rolling OLS estimator, \textrm{avar}$\left( \sqrt{R}\left( \widehat{\beta}_{i,\tau(t),R} -  \beta_{i,\tau(t)}\right) \right)$, by a Newey--West HAC estimator computed over the same $R$-day rolling window:
\begin{align}
...
\begin{align*}
	\widehat{\beta}_{i,\tau(t),R}^{j,U} = \widehat{\beta}_{i,\tau(t),R} + I_{i,\tau(t),R}^j \sup_{i \ s.t. \ I_{i,\tau(t),R}^j=1} \widehat{c}_{LIL,i,\tau(t),R},
\end{align*}
\begin{align*}
	\widehat{\beta}_{i,\tau(t),R}^{j,L} = \widehat{\beta}_{i,\tau(t),R} - I_{i,\tau(t),R}^j \sup_{i \ s.t. \ I_{i,\tau(t),R}^j=1} \widehat{c}_{LIL,i,\tau(t),R}.
\end{align*}
Similarly, let $I_{i,\tau(t),R}^1$ and $I_{i,\tau(t),R}^\mathcal{N}$ be defined according to the same logic %$I_{i,\tau(t),R}^1 = \mathds{1} \left\{ \widehat{\beta}_{i,\tau(t),R} \leq \widehat{\beta}_{\tau(t),R}^{p(1)} \right\}$ and $I_{i,\tau(t),R}^\mathcal{N} = \mathds{1} \left\{ \widehat{\beta}_{\tau(t),R}^{p(\mathcal{N}-1)} < \widehat{\beta}_{i,\tau(t),R} \right\}$
, and define for $j=1, \mathcal{N}$,
\begin{align*}
	\widehat{\beta}_{i,\tau(t),R}^{1,L} = \widehat{\beta}_{i,\tau(t),R} - I_{i,\tau(t),R}^1 \sup_{i \ s.t. \ I_{i,\tau(t),R}^j=1} \widehat{c}_{LIL,i,\tau(t),R},
\end{align*}
\begin{align*}
	\widehat{\beta}_{i,\tau(t),R}^{\mathcal{N},U} = \widehat{\beta}_{i,\tau(t),R} + I_{i,\tau(t),R}^\mathcal{N} \sup_{i \ s.t. \ I_{i,\tau(t),R}^j=1} \widehat{c}_{LIL,i,\tau(t),R},
\end{align*}
...
If Assumptions \ref{ass:B1} and \ref{ass:B2} hold:
\begin{enumerate}[(i)]
\item
\begin{align*}
...
\begin{align*}
    ...
    =&\left( \frac{1}{R}\sum_{s=\tau(t)-R+1}^{\tau(t)}\left( X_{i,s}-\overline{X}_{i,\tau(t),R}\right) \left( X_{i,s}-\overline{X}_{i,\tau(t),R}\right) ^{\top }\right)^{-1} \\
    &\left( \frac{1}{R}\sum_{s=\tau(t) -R+1}^{\tau(t) }\left( X_{i,s}-\overline{X}_{i,\tau(t),R}\right) \left( r_{i,s}-\overline{r}_{i,\tau(t),R}\right) \right),
\end{align*}
...
\begin{align*}
    \widehat{c}_{LIL,i,\tau(t),R}\left( W_t \right) &\equiv - \widehat{c}^L_{LIL,i,\tau(t),R}\left( W_t \right) = \widehat{c}^U_{LIL,i,\tau(t),R}\left( W_t \right) \\
    &= \widehat{\sigma}_{\beta_{0,i},\tau(t),R}\sqrt{\frac{2\ln \ln R}{R}}+ \widehat{\sigma}_{\beta_{1,i},\tau(t),R}\sqrt{\frac{2\ln \ln R}{R}}\left\vert W_t \right\vert.
\end{align*}
...
\end{align*}
Similarly, let $I_{i,\tau(t),R}^1\left( W_t \right)$ and $I_{i,\tau(t),R}^\mathcal{N}\left( W_t \right)$ be defined according to the same logic %$I_{i,\tau(t),R}^1\left( W_t \right) = \mathds{1} \left\{ \widehat{\beta}_{i,\tau(t),R}\left( W_t \right) \leq \widehat{\beta}_{\tau(t),R}^{p(1)}\left( W_t \right) \right\}$ and $I_{i,\tau(t),R}^\mathcal{N}\left( W_t \right) = \mathds{1} \left\{ \widehat{\beta}_{\tau(t),R}^{p(\mathcal{N}-1)}\left( W_t \right) < \widehat{\beta}_{i,\tau(t),R}\left( W_t \right) \right\}$
, and define for $j=1, \mathcal{N}$,
\begin{align*}
...
\begin{propositionBprime}
    \label{prop:B'}
    If Assumptions \ref{ass:B1'} and \ref{ass:B2'} hold, then conditionally on $W_t$:
...
\begin{align*}
    ...
    &\left. -\mathds{1}\left\{ \widehat{\beta }_{\tau(t),R}^{p(\mathcal{N}-1),U}\left( W_t\right) < \widehat{\beta}_{i,\tau(t),R}\left( W_t\right) \right\} \mathds{1}\left\{ \beta _{i,\tau(t) }\left( W_t\right) \leq \widehat{\beta}_{\tau(t),R}^{p(\mathcal{N}-1)}\left( W_t\right) \right\} \right) \\
    &= o_{a.s.}(1)
\end{align*}
\end{enumerate}
...
Let Assumptions \ref{ass:B1} (\ref{ass:B1'}), \ref{ass:B2} (\ref{ass:B2'}), \ref{ass:exogenous} and \ref{ass:condbetas} hold.
\begin{enumerate}[(i)]
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{../../portfolio-sorts/images/data/num_stocks.pdf}
\caption[Number of common stocks in CRSP U.S. stock database]{Number of common stocks in the CRSP U.S. stock database.}
...
\begin{figure}[H]
    ...
    \caption[Average size of (un)trimmed portfolios and trimmed quantity]{Left plot: average size of portfolios across simulations and time; Right plot: average quantity trimmed across simulations ($\mathcal{N}=10, E=12$).}
    \label{img:port_size}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption[Contamination error of (un)trimmed portfolios]{Contamination error with and without trimming for different numbers of portfolios and estimation periods.}
...
Figure \ref{img:betas_dist} displays the empirical distribution of the full-sample beta estimates. The distribution spans a fairly wide range, from about $-0.24$ to $2.13$, and is mildly right-skewed, with a longer upper tail than lower tail. Most estimated betas are positive, although a small fraction are negative, indicating that the bulk of stocks comove positively with the market while a limited number exhibit negative market exposure.

\begin{figure}[H]
\centering
\includegraphics[width=0.55\textwidth]{../../portfolio-sorts/images/estim/bal/betas_dist.pdf}
\caption[Empirical distribution of market betas]{Empirical distribution of the sorting variable estimated on the whole sample.}
\label{img:betas_dist}
\end{figure}
...
\end{align*}
Because the same common shift $\beta_W W_{t,s}$ is added to all stocks, the cross-sectional ordering induced by $\beta_{0,i}$ is preserved over time. Moreover, the residual component is Gaussian and independent across $i$ and $t$:
\begin{align*}
    \varepsilon_{i,t,s} \sim N\left(0,\sigma_{\varepsilon,i}^{2}\right),
\end{align*}
where $\sigma_{\varepsilon,i}$ equals a baseline scale outside the boundary bands and an inflated multiple of that scale inside $\mathcal{B}_2 \cup \mathcal{B}_{\mathcal{N}-1}$. Finally, under $H_{0}$ we set:
    \begin{align*}
        \alpha_{i,t,s}
        =
        -\mu_M \beta_{i,t,s}
...
Note that in the $H_{0}$ design, the additional alpha shifts are confined to narrow boundary bands in portfolios $2$ and $\mathcal{N}-1$, so that the bottom and top portfolios continue to have the same expected return in the absence of sorting errors. In the $H_{A}$ design, instead, the alpha shifts are negative in portfolio $1$ and positive in portfolio $\mathcal{N}$, with larger opposite-signed adjustments in the adjacent boundary bands, so that expected returns increase with beta while contamination from the neighboring portfolios attenuates the bottom-minus-top spread. The weak-correlation design keeps the true beta fixed over time, whereas the strong-correlation design makes it depend on the common state variable $W_{t,s}$, thereby inducing strong cross-sectional dependence in the sorting variable.


\paragraph{Subsampling scheme} We set $R=252$ days (12 months), which implies a ratio $\frac{T-R}{R} = 59$. To maintain the same speed of growth of $T-R$ relative to $R$ in the subsampling scheme, we set $T_\delta = 5\,040$ days (20 years) and $R_\delta = 84$ days (7 months). We thus obtain 481 overlapping windows in the subsampling scheme. We now outline the steps to compute the test statistic $\widehat{Z}_{N,T,R,s}$, the subsampled counterpart $\widehat{Z}_{N,T_\delta,R_\delta,s}^{(k)}$, and recover the size and power of the test statistic. 

First, we recompute the market beta of each stock at each rebalancing date using data for the past $R$ days. In the remainder of this section, $\widehat{S}_{i,\tau(t),R,s} \equiv \widehat{\beta}_{i,\tau(t),s}$, i.e. $\widehat{\beta}_{i,\tau(t),s}$ is taken to be the sorting variable estimated with measurement error.  
...
Next, we implement the subsampling procedure. We proceed as outlined above to recompute the market beta of the stocks, sort them into trimmed portfolios at each rebalancing date, and obtain a time series for the return of the $1$-st trimmed portfolio minus the return of the $\mathcal{N}$-th trimmed portfolio from which we compute the statistic. The only difference is that instead of considering the whole sample, we now consider overlapping windows of $T_\delta$ observations (shifting them by one month at the time), and that we now use an estimation period of $R_\delta$ days as opposed to $R$. For each subsample, we compute the test statistic $\widehat{Z}_{N,T_\delta,R_\delta}^{(k)}$ as the average of the return difference multiplied by the square root of the number of observations.
\begin{align*}
...
\label{Empirical Example}

We now apply the beta-sorting procedure directly to the historical returns. We use the same CRSP and Fama--French data described above. At each monthly rebalancing date, we estimate the market beta of each stock from regression \eqref{reg_estim}, using rolling windows of length $R=21E$, with $E \in \{12,24,60\}$. We then sort stocks into $\mathcal{N}\in\{2,5,10\}$ portfolios according to the estimated beta. For each specification, we construct both the untrimmed portfolios and the trimmed portfolios, where trimming is based on the same law-of-iterated-logarithm bound used in the simulation section. Finally, we compute the equally weighted return of the lowest-beta and highest-beta portfolios over the following month, and report the corresponding high-minus-low beta-sorted statistic. Thus, negative values indicate that the high-beta portfolio earns a lower average return than the low-beta portfolio.

\input{../../portfolio-sorts/tables/application/application.tex} 

Table \ref{tab:application} reports the empirical statistics. Across all specifications, the high-minus-low statistic is negative, consistent with the well-known weak empirical performance of high-beta stocks relative to low-beta stocks. More importantly for our purposes, trimming moves the statistic farther away from zero in every case. For example, when $E=12$ and $\mathcal{N}=10$, the statistic decreases from $-0.068$ without trimming to $-0.097$ with trimming; when $E=24$ and $\mathcal{N}=10$, it decreases from $-0.065$ to $-0.090$. The same pattern is present, though less pronounced, for $\mathcal{N}=2$ and $\mathcal{N}=5$.

This behavior is exactly what the classification-error analysis suggests. Stocks close to the beta cutoffs are the most likely to be assigned to the wrong extreme portfolio. Since these boundary stocks tend to have returns closer to those of adjacent portfolios, their inclusion attenuates the return spread between the two extremes. By removing the observations whose portfolio assignment is least reliable, the trimming procedure delivers cleaner extreme portfolios and strengthens the empirical beta-sorted spread. The effect is largest when $\mathcal{N}=10$, where the extreme portfolios are narrowest and therefore most sensitive to boundary misclassification, and it becomes smaller as the estimation window increases from $E=12$ to $E=60$, when beta estimates are more precise.
...
This paper studies how sorting errors affect tests for equality of expected returns across portfolios. When the sorting characteristic is observable, the relevant error comes from using order statistics to approximate the unobservable population quantiles. We show that this error is asymptotically negligible only when the sorting variable is weakly cross-sectionally correlated; otherwise, it contributes to the asymptotic variance and has to be accounted for in inference. When the sorting characteristic is unobservable and must be estimated, measurement error creates contamination and information-loss errors in the resulting portfolios. The former occurs when a portfolio includes stocks that should belong to another portfolio, while the latter occurs when stocks that should belong to a given portfolio are excluded from it. These classification errors can distort the bottom-minus-top return spread and lead to incorrect inference.

We propose an adaptive trimming procedure that discards stocks close to the portfolio boundaries, where classification errors are most likely to occur. The trimming rule is designed to control contamination while requiring conditions under which the information loss induced by trimming is asymptotically negligible. Because the additional variance terms generated by estimated sorting variables are not generally available in closed form, we combine trimming with a subsampling procedure that preserves the relative growth rate of the testing and estimation windows. The theoretical results show that the resulting tests have correct asymptotic size and unit power. In simulations calibrated to beta-sorted portfolios, trimming substantially reduces contamination, and trimming coupled with subsampling moves rejection frequencies toward the nominal size under the null and toward unit power under the alternative.

We also apply the procedure directly to historical beta-sorted portfolios. At each monthly rebalancing date, we estimate market betas over rolling windows, sort stocks into beta portfolios, and compare the high-minus-low return statistic with and without trimming. The empirical results show that trimming strengthens the beta-sorted spread, especially when the number of portfolios is larger and the extreme portfolios are more sensitive to boundary misclassification.
